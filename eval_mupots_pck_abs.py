...
for ts in range(20):
    ...
    data = np.zeros((num_frames, num_person, 3, 17))
    ...
    for i in range(num_frames):
        ...
        for k in range(len(matches)):
            pred_considered = 0
            if matches[k]!=-1:
                predD, gtD = util.load_dep_pred.get_pred_gt(ts, matches[k], i)
    ...
    savepath = os.path.join(dir_path, test_annot_base, f'TS{ts+1}', f"MuPots_TS{ts+1}_camCentric.mat")
    
    mdic = {"data": data, "label": f"MuPots_TS{ts+1}_camCentric"}
